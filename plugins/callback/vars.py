# Based on roots/trellis, licensed under the MIT license.
...
class CallbackModule(CallbackBase):
    ...
    def raw_triage(self, key_string, item, patterns):
        ...
        if isinstance(item, AnsibleMapping):
            return AnsibleMapping(
                dict(
                    (key, self.raw_triage(".".join([key_string, key]), value, patterns))
        ...
        # process list values
            ...
            return AnsibleSequence(
                [
                    self.raw_triage(".".join([key_string, str(i)]), value, patterns)
        ...
        # wrap values if they match raw_vars pattern
            ...
            match = next(
                (pattern for pattern in patterns if re.match(pattern, key_string)), None
            )
    ...
    def raw_vars(self, play, host, hostvars):
        ...
        raw_vars = Templar(variables=hostvars, loader=play._loader).template(
            hostvars["raw_vars"]
        )
        ...
        patterns = [
            re.sub(r"\*", "(.)*", re.sub(r"\.", r"\.", var))
            for var in raw_vars
            if var.split(".")[0] in hostvars
        ]
    ...
    def cli_options(self):
        ...
        for option, value in iteritems(strings):
            if self._options.get(value, False):
                options.append(
                    "{0}='{1}'".format(option, str(self._options.get(value)))
                )
    ...
    def v2_playbook_on_play_start(self, play):
        ...
        env_group = next(
            (
                group
                for key, group in iteritems(
                    play.get_variable_manager()._inventory.groups
                )
                if key == env
            ),
            False,
        ...
        for host in play.get_variable_manager()._inventory.list_hosts(play.hosts[0]):
            ...
            host.vars["cli_ask_become_pass"] = self._options.get(
